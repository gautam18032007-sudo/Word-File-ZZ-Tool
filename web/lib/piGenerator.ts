...
export async function generatePiWorkbook(input: PiGeneratorInput): Promise<PiGeneratorOutput> {
  const {
    piNumber,
    date,
    buyerName,
...
export async function generatePiPdf(input: PiGeneratorInput): Promise<Buffer> {
  const result = await generatePiWorkbook(input);
  if (!result.pdfBuffer) {
    throw new Error("PDF conversion available only in local environment.");
  }
